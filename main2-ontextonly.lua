...
function DictionaryMode:getContentRect()
    -- Auto: only left/right page margins. Full height.
    -- Ragged line ends are handled later by the word-box hit test.
...
-- CREngine snaps whitespace taps to the nearest word on purpose.
-- Trust the word only if the tap is actually inside that word's box
-- (horizontal check is what fixes ragged left/right aligned lines).
...
function DictionaryMode:onTap(_, ges)
    ...
    local text = self:cleanupSelectedText(selection.text or word.word)
    if text == "" then
        return false
    end
